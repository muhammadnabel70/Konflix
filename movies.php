...
// 1. BANNER SLIDESHOW (Ambil 5 FILM Acak)
$q_hero = mysqli_query($conn, "SELECT * FROM contents WHERE type='movie' ORDER BY RAND() LIMIT 5");
$hero_items = [];
while($row = mysqli_fetch_assoc($q_hero)) { $hero_items[] = $row; }

// 2. GRID KONTEN (Semua Film)
$q_movies = mysqli_query($conn, "SELECT * FROM contents WHERE type='movie' ORDER BY id DESC");
?>

<!DOCTYPE html>
<html>
<head>
    <title>Movies - SecureStream</title>
            ...
            <?php foreach($hero_items as $index => $item): ?>
                <div class="hero-slide <?php echo ($index == 0) ? 'active' : ''; ?>" style="background-image: url('<?php echo $item['poster_landscape']; ?>');">
                    ...
                    <div class="hero-content">
                        ...
                        <div class="hero-meta">
                            <?php echo substr($item['release_date'], 0, 4); ?> • 
