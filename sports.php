...
// 1. BANNER SLIDESHOW (Ambil 5 SPORT Acak)
$q_hero = mysqli_query($conn, "SELECT * FROM contents WHERE type='sport' ORDER BY RAND() LIMIT 5");
$hero_items = [];
while($row = mysqli_fetch_assoc($q_hero)) { $hero_items[] = $row; }

// 2. GRID KONTEN
$q_sports = mysqli_query($conn, "SELECT * FROM contents WHERE type='sport' ORDER BY id DESC");
?>

<!DOCTYPE html>
<html>
<head>
    <title>Sports - SecureStream</title>
...
<div class="app-container">
    ...
    <div class="main-content">
            ...
            <?php foreach($hero_items as $index => $item): ?>
                <div class="hero-slide <?php echo ($index == 0) ? 'active' : ''; ?>" style="background-image: url('<?php echo $item['poster_landscape']; ?>');">
                    ...
                    <div class="hero-content">
                        ...
                        <div class="hero-meta">Live & Replays • Sport Event</div>
                        <p class="hero-desc"><?php echo substr($item['description'], 0, 180); ?>...</p>
                        <div class="btn-group">
                            <a href="watch.php?v=<?php echo $hasher->encode($row['id']); ?>" class="btn-hero btn-play"><i class="fa-solid fa-play"></i> TONTON</a>
        ...
        <h1 class="page-title">Pertandingan & Replay</h1>
